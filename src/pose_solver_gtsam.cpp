...
gtsam::Pose3 PoseSolverGtsam::solve(const gtsam::Pose3& pose_initial_target_cam,
  ...
  graph.addPrior(gtsam::Symbol('x', 0), pose_initial_target_cam, options_.pose_prior_noise_model);
  ...
  for (const auto& meas : measurements) {
    graph.emplace_shared<gtsam::GenericProjectionFactor<gtsam::Pose3, gtsam::Point3, gtsam::Cal3Fisheye>>(
        meas.uv, options_.pixel_meas_noise_model, gtsam::Symbol('x', 0), gtsam::Symbol('l', meas.point_id),
        cmod_params);
  }

  // Add landmark priors to graph.
  for (const auto& meas : measurements) {
    graph.addPrior(gtsam::Symbol('l', meas.point_id), pts3d_target.at(meas.point_id),
                   options_.landmark_prior_noise_model);
  }

  // Create initial estimate for landmarks and camera pose.
  gtsam::Values initial_estimate;
  for (const auto& meas : measurements) {
    initial_estimate.insert<gtsam::Point3>(gtsam::Symbol('l', meas.point_id), pts3d_target.at(meas.point_id));
  }
  initial_estimate.insert<gtsam::Pose3>(gtsam::Symbol('x', 0), pose_initial_target_cam);
  ...
  return result.at<gtsam::Pose3>(gtsam::Symbol('x', 0));
}
